...
Print n lines where each line  i (in the range 1 <= i <= n) contains the respective decimal, octal, capitalized hexadecimal, and binary values of .
...
def print_formatted(number):
    spaces= len(str(bin(number)))-2
    for i in range(1,number+1):
        octal = oct(i)[2:]
        hexa  = str(hex(i)[2:]).upper()
        binary= bin(i)[2:]
        print(str(i).rjust(spaces)+octal.rjust(spaces+1)+hexa.rjust(spaces+1)+binary.rjust(spaces+1))
